...
function DismissButton({ onClick, title }: { onClick: () => void; title: string }) {
  return (
    <button
      ...
      title={title}
      aria-label={title}
      style={{ width: 24, height: 24, padding: 0, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "none", borderRadius: 0, background: "none", color: "var(--text-dim)", cursor: "pointer" }}
  ...
  return (
    <div style={{ minHeight: "100%" }}>
      ...
      {showUploadFeedback && (
        ...
        {pendingConflict && (
          <div role="alert" style={{ padding: 7, border: "1px solid color-mix(in srgb, var(--tile-amber) 55%, var(--border))", borderRadius: 0, background: "color-mix(in srgb, var(--tile-amber) 9%, var(--bg-panel))" }}>
            ...
            {pendingConflict.nonReplaceable.length > 0 && (
              <div style={{ marginTop: 3, fontSize: 10, color: "var(--tile-amber)", lineHeight: 1.35, overflowWrap: "anywhere" }}>
                Cannot replace: {pendingConflict.nonReplaceable.join(", ")}
              </div>
            )}
            <div style={{ display: "flex", gap: 5, marginTop: 7 }}>
              <button type="button" onClick={() => void performUpload(pendingConflict.files, "overwrite")} style={{ height: 22, padding: "0 7px", border: "none", borderRadius: 0, background: "var(--tile-red)", color: "#fff", cursor: "pointer", fontSize: 10, fontWeight: 600 }}>
                Replace
              </button>
              <button type="button" onClick={() => void performUpload(pendingConflict.files, "skip")} style={{ height: 22, padding: "0 7px", border: "none", borderRadius: 0, background: "var(--tile-steel)", color: "#fff", cursor: "pointer", fontSize: 10, fontWeight: 600 }}>
                Skip existing
              </button>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, minHeight: 22, fontSize: 11 }}>
              <div style={{ minWidth: 0, flex: 1, display: "flex", alignItems: "center", gap: 8 }}>
                {uploadSummary.uploaded.length > 0 && (
                  <span title={`${uploadSummary.uploaded.length} uploaded`} aria-label={`${uploadSummary.uploaded.length} uploaded`} style={{ display: "flex", alignItems: "center", gap: 3, color: "var(--tile-green)" }}>
