...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onAtMention }: Props) {
  ...
  useEffect(() => {
    ...
    if (selectedCwd === null) {
      ...
      const projects = getRecentProjects(allSessions).filter((c) => !hiddenCwds.has(c));
  ...
  const recentProjects = getRecentProjects(allSessions).filter((p) => !hiddenCwds.has(p));
                ...
                {visibleProjects.map((project) => (
                  <div
                    ...
                    style={{
                      ...
                      width: "100%",
                      padding: "8px 10px",
                      background: project === selectedProject ? "var(--bg-selected)" : "none",
                      borderBottom: "1px solid var(--border)",
                  ...
                  >
                    ...
                    {project !== selectedProject && (
                      <button
                        onClick={(e) => { e.stopPropagation(); hideCwd(project); }}
                        title="从最近列表移除（不会删除会话）"
                        aria-label={`从最近列表移除 ${project}`}
                        style={{
                          flexShrink: 0,
                          display: "flex", alignItems: "center", justifyContent: "center",
    ...
    <div
      ...
      style={{
        ...
        background: confirmDelete
          ? "rgba(239,68,68,0.06)"
          : isSelected ? "var(--accent)" : hovered ? "var(--bg-hover)" : "transparent",
      ...
      {confirmDelete ? (
        ...
        <>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ marginTop: 2, display: "flex", gap: 8, color: isSelected ? "rgba(255,255,255,0.82)" : "var(--text-dim)", fontSize: 11, minWidth: 0 }}>
          ...
          {hasChildren && (
            <button
              ...
              style={{
                ...
                background: "none", border: "none",
                color: isSelected ? "rgba(255,255,255,0.85)" : "var(--text-dim)", cursor: "pointer",
