...
## 2026-06-23 · 全屏忠实还原 pass（分支 `redesign/hifi-align-pass`）

**用户决策**: 「全部忠实还原」设计稿（不做移动端），**推翻 iter 阶段为可用性做的 3 处刻意偏离**（会话列表 / 紧凑抬头 / 静态工具磁贴）。逐项 dev 截图验证、console 零报错。

- [x] **A. 大号 π 英雄区（屏③ 新会话）** — `ChatWindow` isEmptyNew：紧凑单行抬头 → 居中 `clamp(96–150px)` cobalt π + `clamp(34–52px)` 细体(weight200) "Pi Agent" + 18px Typewriter(cobalt 光标) + 输入卡 + 页脚 `web vX · pi vX · Enter 发送`。inspect: π #0050EF / Segoe UI ✓，截图 ✓。
- [x] **B. 冷启动/无会话 ghost-π（屏①②）** — `AppShell` placeholder：左上 "Get Started" → 居中 ghost-π(`var(--bg-selected)`)。无 cwd=π+"Pi Agent"+"请先在左侧选择工作目录"+2步引导；有 cwd 无会话=π+"从左侧会话磁贴墙选择…＋New"。
- [x] **C. 会话彩色磁贴墙（屏②）** — `SessionSidebar`：列表(SessionItem 54px 行) → 双列彩色磁贴(84px)。`flattenTree` 扁平化分叉树(fork=↳+图标)；`tileColor`(id hash→8 色 WP 调色板)；选中=cobalt+白内描边；运行中=cobalt+pulseGlow+dotPulse（经 `AppShell.onAgentRunningChange`→`ChatWindow` agentRunning→`selectedRunning` 接通）；msg 角标 top-right；相对时间 bottom；hover 出重命名/删除，删除=磁贴内 scrim 二次确认。**功能 1:1**（选/改名/删/分叉关系全保留）。截图：34 块彩色磁贴 + "会话·34" 头 ✓。旧 `SessionItem`/`SessionTreeItem`(285 行)已删（eslint no-unused-vars=error 会让 build 挂）。
- [x] **D. 工具调用翻面磁贴（屏④）** — `MessageView.ToolCallBlock`：teal 头做成 3D 翻面磁贴，正面=工具名/预览/时长，背面=`✓/✗ N行 · 结果首行 · 时长`，**hover 翻面**（非设计稿的无限自动循环——长会话几十个工具磁贴永久翻转=噪声，故改 hover 触发；运行中显 dotPulse；点击仍展开完整 input/result）。截图确认正面 + hover 翻出背面结果摘要 ✓。
- [x] **E. 精修 + 浅色走查** — ① cwd 选择器补设计稿②的文件夹图标(有cwd=cobalt/无=amber) + ▾ 箭头；② 助手消息 accent 竖条 3px→4px 对齐设计稿（保留无面板底——长会话+工具磁贴防杂乱的既定取舍）。**浅色主题走查 ✓**：#F2F2F2 画布 + 同套彩色会话磁贴 + teal 工具翻面 + cobalt 用户块全部正常（可换肤红线保留）。Models(⑥)/Skills(⑦) 弹窗复查 ✓（provider/skill 选中=cobalt、violet T 徽章、绿 toggle、API Key 掩码）。屏⑤文件面板未本轮重截（未改动，iter7 已验）。
- [x] **F. 构建安全校验** — `npx tsc --noEmit` 退出 0（无类型错误）；`npx eslint` 四个改动组件退出 0（无 unused-vars，删 SessionItem 后无残留）。→ 正式 `next build` 类型/lint 这关应过。正式 build + 重打桌面包仍待用户。

**坑记录**: Bash `mv` 重排文件后 Turbopack 文件监听未触发 → dev 服务跑陈旧编译，报 `flattenTree is not defined`(陈旧行号)。**教训：大改/重排后若 HMR 报与当前源不符的行号错误，用 `preview_stop`+`preview_start` 干净重编译，别反复 reload。**
